...
const char * const BITCOIN_CONF_FILENAME = "truefreedomcoin.conf";
...
#else
    const char* pszModule = "truefreedomcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TrueFreedomCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/TrueFreedomCoin";
#else
    // Unix
    return pathRet / ".truefreedomcoin";
#endif
#endif
}
