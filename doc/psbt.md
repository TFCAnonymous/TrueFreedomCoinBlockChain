# PSBT Howto for TrueFreedomCoin Core

Since TrueFreedomCoin Core 0.17, an RPC interface exists for Partially Signed TrueFreedomCoin
...
PSBT is an interchange format for TrueFreedomCoin transactions that are not fully signed
...
Overall, the construction of a fully signed TrueFreedomCoin transaction goes through the
following steps:
...
- An **Extractor** produces a valid TrueFreedomCoin transaction (in network format)
...
#### Multisig with multiple TrueFreedomCoin Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
TrueFreedomCoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `truefreedomcoin-cli` is used.
